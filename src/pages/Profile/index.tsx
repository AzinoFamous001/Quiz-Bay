...
// Key for storing streak data in localStorage
...
  // Initialize streak from localStorage immediately
...
    let foundQuizTodayOrYesterday = false;

    // 3. Check for streak continuation (Did the user complete a quiz today or yesterday?)

    // A. Check for TODAY's quiz
    const todayStr = currentDate.toISOString().split('T')[0];
    if (quizDates.has(todayStr)) {
      streak = 1; // Started or continued the streak today
      currentDate.setDate(currentDate.getDate() - 1); // Move to yesterday for the loop
      foundQuizTodayOrYesterday = true;
    } else {
      // B. Check for YESTERDAY's quiz
      const yesterday = new Date(today);
      yesterday.setDate(yesterday.getDate() - 1);
      const yesterdayStr = yesterday.toISOString().split('T')[0];
      
      if (quizDates.has(yesterdayStr)) {
        // User completed yesterday but not today. Streak is 1 for yesterday.
        streak = 1;
        currentDate = yesterday;
        foundQuizTodayOrYesterday = true;
      } else {
        // No quiz today or yesterday. Streak is broken or never started.
        localStorage.setItem(STREAK_KEY, JSON.stringify({ streak: 0, lastCheckDate: todayStr }));
        return 0;
      }
    }
    
    // 4. Loop backwards to find the maximum consecutive days
    while (foundQuizTodayOrYesterday) {
      // Check the day before currentDate
...
    }
    
    // 5. Store the final calculated streak
...
  // 3. Recalculate streak when results change
  useEffect(() => {
...
  const streak = currentStreak; // Use the state value
...
    if (!acc[title]) {
      acc[title] = { name: title, quizzes: 0, totalPercentage: 0 };
    }
    acc[title].quizzes += 1;
...
              <p className="text-gray-200 text-lg">{currentUser.email}</p>
              <div className="flex flex-wrap gap-4 mt-4 justify-center md:justify-start">
                <span className="bg-gradient-to-r from-yellow-500 to-orange-500 px-5 py-2 rounded-full text-white font-bold">
                  {stats.rank} Rank
                </span>
                <span className="bg-gradient-to-r from-red-600 to-orange-500 px-5 py-2 rounded-full text-white font-bold flex items-center gap-2 shadow-lg">
...
          ].map((stat, i) => (
            <div key={i} className="bg-white/10 backdrop-blur-lg rounded-2xl p-6 border border-white/20 text-center hover:bg-white/15 transition">
...
        {/* Tabs */}
        <div className="flex gap-4 mb-6 bg-white/10 backdrop-blur-lg rounded-xl p-2">
...
            ) : (
              results.map(result => (
                <div key={result.id} className="bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20 flex items-center justify-between hover:bg-white/15 transition">
                  <div className="flex items-center gap-6">
                    <div className="bg-blue-500/20 p-4 rounded-xl">
                      <BookOpen className="w-8 h-8 text-blue-400" />
                    </div>
                    <div>
                      <h3 className="text-xl font-bold text-white">{result.quizTitle}</h3>
